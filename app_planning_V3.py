...
# -*- coding: utf-8 -*-
"""
Application de planning pour juges CrossFit - Unicorn Throwdown 2025
"""
...
def load_logo():
    try:
        ...
        st.warning("Logo non trouvé. Placez 'logo.png' dans le même dossier que l'application.")
        return None
    except Exception as e:
...
st.title("Planning des Juges - Unicorn Throwdown 2025")

# Fonction de génération PDF pour les juges
def generate_pdf_tableau(planning: Dict[str, List[Dict[str, any]]]) -> FPDF:
    pdf = FPDF(orientation='P')
    pdf.set_auto_page_break(auto=True, margin=15)
...
# Fonction de génération PDF pour les heats
def generate_heat_pdf(planning: Dict[str, List[Dict[str, any]]]) -> FPDF:
    heat_map = defaultdict(lambda: defaultdict(str))
    ...
    for i in range(0, len(heats), 2):
        ...
        for j in range(2):
            ...
            pdf.cell(0, row_height, f"HEAT - {start} à {end}", ln=1)
            
            pdf.set_font("Arial", '', 10)
            pdf.set_xy(10, y_position + row_height)
            pdf.cell(0, row_height, f"WOD: {wod} | Emplacement: {location}", ln=1)
            ...
            for lane in sorted(lanes.keys()):
                ...
                pdf.cell(col_width/2, row_height, str(lane), border=1, align='C', fill=True)
                pdf.cell(col_width/2, row_height, lanes[lane], border=1, align='C', fill=True)
                pdf.ln()
                fill = not fill
